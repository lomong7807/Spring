...
public interface User1Repository extends JpaRepository<User1Entity, String>{
	...
	// JPQL
	@Query("select u1 from User1Entity as u1 where u1.age < 30") // entity 별칭을 지정하고 그 별칭을 조회
	public List<User1Entity> selectUser1UnderAge30();
	
	@Query("select u1 from User1Entity as u1 where u1.name = ?1") // entity 별칭을 지정하고 그 별칭을 조회
	public List<User1Entity> selectUser1ByName(String name);
	
	@Query("select u1 from User1Entity as u1 where u1.name = :name") // entity 별칭을 지정하고 그 별칭을 조회, 파라미터로 바인딩
	public List<User1Entity> selectUser1ByNameParam(@Param("name") String name);
	
	@Query("select u1.uid, u1.name, u1.age from User1Entity as u1 where u1.name = :uid") // entity 별칭을 지정하고 그 별칭을 조회, 파라미터로 바인딩
